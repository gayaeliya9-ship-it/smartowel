...
database.ref('fromAltera').on('value', (s) => {
    ...
    if(d) {
        ...
        if (currentHomeState.alarmArmed && dist <= 10 && dist > 0) {
            // התנאי התקיים: האזעקה דרוכה והבאזר הפיזי מצפצף עכשיו (מרחק 1-10 ס"מ)
            ...
            // הקפצת החלון המרכזי במידה ועדיין לא קפץ
            if (intruderModal && !isIntruderShown) { 
                isIntruderShown = true; 
                if(smartModal) smartModal.hide(); 
                isAlertShown = false; 
                document.getElementById('intruderDistVal').innerText = dist; 
                intruderModal.show(); 
            }
            
        } else if (!isIntruderShown) {
            // התוספת הקריטית: מסתירים ומאפסים רק אם חלון הפריצה לא פתוח כרגע
            document.getElementById('alarmWarning').style.display = "none";
            if (!currentHomeState.alarmArmed) {
                document.getElementById('alarmWrapper').classList.remove('alarm-active');
            }
